...
class DailyReservationNormalizer:
    ...
    def add_others(self, df: pd.DataFrame) -> None:
        ...
        # 時刻設定を行う
        is_not_null1  =df["出発時刻"].notnull() 
        is_not_null2  =df["到着時刻"].notnull() 
        is_not_empty1 = df["出発時刻"].astype(str).str.strip() != ""
        is_not_empty2 = df["到着時刻"].astype(str).str.strip() != ""
        Condition1 = is_not_null1 & is_not_empty1
        Condition2 = is_not_null2 & is_not_empty2
