...
def layout_conv_domestic(df) -> list[ReadResult] :
    ...
    for start_row in range(start_index, len(df), block_size):
        ...
        for col_idx in date_cols:
            ...
            if pd.isna(block.iloc[0, col_idx]) or str(block.iloc[0, col_idx]).strip() == "":
                continue
    ...
    # 3. データフレーム化
    results = []
    results.append(ReadResult(df=pd.DataFrame(records),layout=Layout_type.DAILY.value))
    return results

def layout_conv_domestic2(df) -> list[ReadResult] :
    ...
    # 2. 便ごとのデータブロックを6行単位でループ処理
    start_index = 4     #　EXCEL5行目から開始
    block_size = 7      #　7行単位で取得する
    #　Range(開始,終了,ステップ) len(df)はDFの行数なので、4行目からDFの行数まで、6行ずつ
    for start_row in range(start_index, len(df), block_size):
        if start_row + block_size > len(df):
            break
        #　(block)範囲抽出データには4行目から10行目（6行分）のデータをひとかたまりで生成する    
        block = df.iloc[start_row : start_row + block_size]
        ...
        for col_idx in date_cols:
            ...
            record = {
                ...
                '座席数': block.iloc[1, col_idx], # 2行目
                '旅客数': block.iloc[2, col_idx], # 3行目
                '日本人数': block.iloc[3, col_idx], # 3行目
                'INF数': block.iloc[4, col_idx],  # 4行目
                '貨物重量': block.iloc[5, col_idx],# 5行目
                'メール重量': block.iloc[6, col_idx], # 6行目
                '備考':'',
                '事業所': jigyosho
            }
            records.append(record)
    ...
    # 3. データフレーム化
    results = []
    results.append(ReadResult(df=pd.DataFrame(records),layout=Layout_type.DAILY2.value))
    return results
...
def layout_conv_inter(df) -> pd.DataFrame:
    ...
    # 3. データフレーム化
    results = []
    results.append(ReadResult(df=pd.DataFrame(records),layout=Layout_type.DAILY3.value))
    return results
