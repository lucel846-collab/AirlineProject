...
def validate_previous_date_check_daily(df: pd.DataFrame, result: ValidationResult) -> None:
...
def validate_previous_date_check_monthly(df: pd.DataFrame, result: ValidationResult) -> None:
    today = pd.Timestamp.today().normalize()
    first_date = today.replace(day=1)
    last_date_prev_month = first_date - pd.Timedelta(days=1)
    first_date_prev_month = last_date_prev_month.replace(day=1)
